<template>
  <div class="study-grid h-auto mb-12">
    <div class="desktop:col-start-1 desktop:col-end-13 col-span-full text-left">
      <h3 class="h3-bold text-gray-900 mb-7">Research</h3>
      <div class="desktop:col-start-1 desktop:col-end-7">
        <h4 class="h4-medium text-gray-900">Diagnosing Possible Issues</h4>
        <div class="sm:px-3 sm:pl-8 sm:pr-4 mt-2">
          <p class="paragraph-mobile-small desktop:paragraph-regular">
            First and foremost, I opened up GABO Online and started taking
            screenshots, while using the site as a user would, from start to
            finish. I’ve even bought a book to see how the checkout process
            ended. Next, on my Miro board, I’ve highlighted all the issues I
            could find.
            <a
              href="https://miro.com/app/board/o9J_loXL4Jk=/?moveToWidget=3074457366784305196&cot=14"
              class="text-link"
              target="_blank"
              >Feel free to check it out in detail.</a
            >
          </p>
          <p class="paragraph-mobile-small desktop:paragraph-regular">
            All in all, I’ve found the following kinds of issues:
          </p>
          <div class="sm:px-3 pl-8 sm:pl-14 sm:pr-4 mt-2">
            <div class="">
              <ul
                class="
                  paragraph-mobile-small
                  desktop:paragraph-regular
                  list-disc
                "
              >
                <li>Visual glitches</li>
                <li>Unclear copy</li>
                <li>Cluttered & illogical design</li>
                <li>Issues with font sizes and colours</li>
                <li>Confusing forms</li>
                <li>
                  Easily missable features (authors dropdown, e-books banner,
                  user profile)
                </li>
                <li>
                  No feedback given (I’ve requested two out of print books, but
                  I haven’t received even an automatic acknowledgement about it,
                  and the user profile has no option to check if I’ve requested
                  a book or not)
                </li>
                <li>
                  Too many fonts: Arial, Helvetica, Verdana, Times New Roman,
                  Questrial Regular. For comparison: competitors like Libri and
                  Bookline have only one (technically Libri has two, but as far
                  as I know, they only use it for one sentence on the product
                  page).
                </li>
              </ul>
            </div>
          </div>

          <p class="paragraph-mobile-small desktop:paragraph-regular">
            I thought there were some nicer design choices as well:
          </p>

          <div class="sm:px-3 pl-8 sm:pl-14 sm:pr-4 mt-2">
            <div class="">
              <ul
                class="
                  paragraph-mobile-small
                  desktop:paragraph-regular
                  list-disc
                "
              >
                <li>
                  The footer adheres to accessibility standards in its colours
                  (although the font size is a bit too small)
                </li>
                <li>The “My Orders” page is quite nice and useful</li>
                <li>Sorting options at search results seemed acceptable</li>
                <li>
                  The publisher’s blog is not the most beautiful, but it serves
                  its purpose
                </li>
                <li>Authors dropdown is a nice feature</li>
              </ul>
            </div>
          </div>
        </div>
      </div>

      <div class="desktop:col-start-1 desktop:col-end-7">
        <h4 class="h4-medium text-gray-900">Usability Testing</h4>
        <div class="sm:px-3 sm:pl-8 sm:pr-4 mt-2">
          <h5 class="h5-bold mb-4 mt-6 text-gray-900">Method</h5>
          <p class="paragraph-mobile-small desktop:paragraph-regular">
            Next, I facilitated quite long (1.5-2hrs) and detailed usability
            testing sessions over Zoom to find out what other people’s pain
            points and needs are when encountering this webshop. These sessions
            started with a short user interview, where we talked about how
            participants shop online in general, what kind of books they like,
            from where they buy them etc., then we started testing the site,
            from sign-up to checkout.
          </p>
        </div>
        <div class="sm:px-3 sm:pl-8 sm:pr-4 mt-2">
          <h5 class="h5-bold mb-4 mt-6 text-gray-900">Participants</h5>
          <p class="paragraph-mobile-small desktop:paragraph-regular">
            I recruited 6 participants between the ages of 29-35. According to
            them, they are confident online shoppers, but never, or almost never
            use their mobiles or tablets to buy something. They have quite
            different needs when it comes to buying books. 4 participants
            weren’t familiar with GABO/Ciceró/Akkord, and 2 participants had
            only an inkling about them — and none of them used this online shop
            previously.
          </p>
          <p class="paragraph-mobile-small desktop:paragraph-regular">
            I should mention it, one of the participants was a fellow UX/UI
            designer, and I was afraid her feedback won’t be as usable as the
            others, since it’s inevitable that a designer will look at a site
            differently than a regular user. However, even though she mentioned
            a lot of small details (good and bad), I was happy when other
            participants mentioned those too. So I kept her insights in my
            research notes.
          </p>
        </div>
        <div class="sm:px-3 sm:pl-8 sm:pr-4 mt-2">
          <h5 class="h5-bold mb-4 mt-6 text-gray-900">Synthesis</h5>
          <p class="paragraph-mobile-small desktop:paragraph-regular">
            After the interviews, I watched the recordings and gathered quite a
            lot of insights, then arranged them on a Miro board to better
            synthesize them, using affinity mapping. It took a long time, but it
            was a necessary step.
          </p>
          <p class="paragraph-mobile-small desktop:paragraph-regular">
            With the tasks I’ve given to participants, I wanted to follow a
            user’s journey, and later I’ve organised these insights by these
            steps as well.
          </p>
          <p class="paragraph-mobile-small desktop:paragraph-regular">
            Feel free to check out the
            <a
              href="https://miro.com/app/board/o9J_loXL4Jk=/?moveToWidget=3074457367725564316&cot=14"
              target="_blank"
              class="text-link"
              >“Usability Testing - Current Site” on my Miro project page</a
            >
            for details.
          </p>
        </div>
        <div class="sm:px-3 sm:pl-8 sm:pr-4 mt-2">
          <h5 class="h5-bold mb-4 mt-6 text-gray-900">
            Insights About Shopping
          </h5>
          <p class="paragraph-mobile-small desktop:paragraph-regular">
            When using an online shop...
          </p>
          <div class="sm:px-3 pl-8 sm:pl-14 sm:pr-4 mt-2">
            <div class="">
              <ul
                class="
                  paragraph-mobile-small
                  desktop:paragraph-regular
                  list-disc
                "
              >
                <li>clean copy and design is high-priority,</li>
                <li>participants need good search and filter options,</li>
                <li>participants want the site to seem trustworthy,</li>
                <li>participants want to see a wide selection of products,</li>
                <li>participants want that site to be easy to use,</li>
                <li>delivery time and price is important,</li>
                <li>participants expect sites to fulfil their promises.</li>
              </ul>
            </div>
          </div>
          <p class="paragraph-mobile-small desktop:paragraph-regular">
            About buying books:
          </p>
          <div class="sm:px-3 pl-8 sm:pl-14 sm:pr-4 mt-2">
            <div class="">
              <ul
                class="
                  paragraph-mobile-small
                  desktop:paragraph-regular
                  list-disc
                "
              >
                <li>
                  Ease of use is more important than buying directly from a
                  publisher.
                </li>
                <li>
                  Participants are price-conscious - wide selection, low
                  delivery fees and discounts are important.
                </li>
                <li>
                  Some users tend to look for books in a brick-and-mortar store
                  first.
                </li>
                <li>
                  Some users mentioned that they didn’t really know smaller
                  chains.
                </li>
                <li>
                  Users most commonly buy from Libri, Bookline, Szellemlovas,
                  Alexandra, Sárkánytűz, Bookdepository.
                </li>
              </ul>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>

  <div class="grid grid-cols-12 gap-4 h-full mb-12">
    <div class="col-span-full">
      <picture>
        <source
          media="(max-width: 639px)"
          srcset="/src/assets/userinfo-vertical@2x.jpg"
        />
        <source
          media="(max-width: 767px)"
          srcset="/src/assets/userinfo-vertical@2x.jpg"
        />
        <img
          class="w-full"
          src="/src/assets/userinfo@2x.jpg"
          alt="Post-it notes showing information about the usability test participants."
        />
      </picture>
    </div>
  </div>

  <div class="study-grid h-auto mb-12">
    <div class="desktop:col-start-1 desktop:col-end-7 col-span-full row-start-1 text-left">
      <div class="sm:px-3 sm:pl-8 sm:pr-4 mt-2">
        <h5 class="h5-bold mb-4 mt-6 text-gray-900">
          Insights About GABO Online
        </h5>
        <p class="paragraph-mobile-small desktop:paragraph-regular">
          Overall, participants rated the current site a strong medium — they
          wouldn’t really recommend it unless the prices were really good.
          They’ve felt uncertain a lot, and they thought the design looked
          outdated.
        </p>
        <p class="paragraph-mobile-small desktop:paragraph-regular">
          Another problem was that the publishing group was mostly unknown to
          them: 4 participants had an interest in fantasy/sci-fi literature, yet
          only one of them mentioned knowledge about GABO’s SFF books. One
          participant has small children, yet she didn’t know Ciceró’s
          childrens’ books, and one other participant likes crime fiction and YA
          books, but she only knew that Ciceró published a YA book series she’s
          read.
        </p>
      </div>
    </div>
    <div class="desktop:col-start-7 desktop:col-end-13 col-span-full row-span-1 self-center">
      <div
        class="
          flex
          items-center
          md:px-12
          sm:px-8
          md:mx-12
          px-4
          h-48
          bg-gray-300
          rounded-md
        "
      >
        <p class="subheading-bold">
          "I'd hope everything will be okay with my book because I wouldn't know
          what to expect otherwise."
        </p>
      </div>
    </div>
    <div class="desktop:col-start-1 desktop:col-end-13 col-span-full text-left">
      <div class="sm:px-3 sm:pl-8 sm:pr-4">
      <p class="paragraph-mobile-small desktop:paragraph-regular">
        Key insights:
      </p>
      <div class="sm:px-3 pl-8 sm:pl-14 sm:pr-4 mt-2">
        <div class="">
          <ul class="paragraph-mobile-small desktop:paragraph-regular list-disc">
            <li>Some of the more fundamental functions frustrated users — each of them had problems with the placement of the user profile. None of them thought to look for it in the footer at first, where it currently is, and for some of them, it took longer to find it.</li>
            <li>The sign-up process has some redundant steps, inconsistent copy, and is sometimes hard to understand and difficult to read. Participants were also missing some ease of use features, like automatically outputting the city name after inputting a ZIP code, or showing how strong a password is. Some features also confused them, since they didn’t follow the Hungarian standard, like asking their first name before their family name, or asking for their date of birth in the DD/MM/YYYY format.</li>
            <li>Not every detail about payment/delivery can be easily found, and these details are not represented well.</li>
            <li>The contact form is in an intuitive location, but it could be more usable. Participants felt that more info could be on this page, e.g. other ways to contact the publisher.</li>
            <li>pNot having any filtering options when searching or browsing caused some headaches to participants. They also felt that the available sorting options are not satisfactory.</li>
            <li>Some users found the current search functionality (autocomplete) and the results page pleasing.</li>
            <li>Participants felt the way the site handles alert requests for out-of-stock books is redundant and frustrating.</li>
            <li>The current product page seems too complex while missing some important features, e.g. clickable author name, or input stepper to select multiple copies.</li>
            <li>Participants felt that the checkout process has some interesting features, but should be more intuitive and organised. Missing CTAs on the payments page made them unsure about how to proceed.</li>
          </ul>
        </div>
      </div>
      </div>
    </div>
  </div>

  <div class="grid grid-cols-12 gap-4 h-full mb-12">
    <div class="col-span-full">
      <picture>
        <source
          media="(max-width: 639px)"
          srcset="/src/assets/affinitymap-vertical@2x.jpg"
        />
        <source
          media="(max-width: 767px)"
          srcset="/src/assets/affinitymap-vertical@2x.jpg"
        />
        <img
          class="w-full"
          src="/src/assets/affinitymap@2x.jpg"
          alt="Post-it notes showing information about the usability test insights."
        />
      </picture>
    </div>
  </div>

  <div class="study-grid h-auto mb-12">
    <div class="desktop:col-start-1 desktop:col-end-13 col-span-full row-start-1 text-left">
      <div class="sm:px-3 sm:pl-8 sm:pr-4 mt-2">
        <h5 class="h5-bold mb-4 mt-6 text-gray-900">
          Customer Service
        </h5>
        <p class="paragraph-mobile-small desktop:paragraph-regular">
          One thing I didn’t tackle in this project was the issue of contacting the publishing group. After checking their websites and socials, there seem to be 4 e-mail addresses and 4 phone numbers a customer can use to contact them, and GABO Online has a Contact Page with a contact form, that has two options: one if you want to write to the customer service, and one if you want to write to the publishers. However, this contact page doesn’t have every phone number and e-mail address listed. And, interestingly, there are contact details in the site’s footer as well.
        </p>
        <p class="paragraph-mobile-small desktop:paragraph-regular">
          The participants of the usability tests were also confused about this, although they only saw the footer and the Contact Page due to the scope of the tests. They also mentioned how much better it would be if the Contact Page contained at least a phone number, an e-mail address, some info about when to expect an answer, and they also felt the contact form wasn’t very intuitive, some of its functions didn’t work at all (like selecting a previous order, even though we were using a live account with very current previous orders — this hasn’t been fixed yet, as of March 26, 2022).
        </p>
        <p class="paragraph-mobile-small desktop:paragraph-regular">
          I think 4 e-mail addresses and the same amount of phone numbers, a Contact Page and several social media pages should be more than enough channels for people to contact GABO, Ciceró or Akkord. So, in a way, that’s good. You can definitely reach them somehow. However, I would examine if this amount of contactability is really necessary. Depending on how the publishing group is structured (and that’s something beyond my knowledge), having only one e-mail address and one phone number, combined with a better Contact Page could be enough. E-mails can be sorted behind the scenes, calls can be redirected, and customers would probably not be confused if they only have one or two options to choose from.
        </p>
      </div>
      <div class="sm:px-3 sm:pl-8 sm:pr-4 mt-2">
        <h5 class="h5-bold mb-4 mt-6 text-gray-900">
          Prioritisation
        </h5>
        <p class="paragraph-mobile-small desktop:paragraph-regular">
          Having a lot of insight is good, however, it soon became overwhelming. I didn’t know where to start with my design. So I’ve prioritized the main issues about the site using <a href="https://maze.co/guides/usability-testing/results/" target="_blank" class="text-link">a technique I’ve read about on Maze</a>: I sorted my summary post-its into five categories, ranging from critical issues to no issues. For some reality check, I’ve asked the help of one of my developer friends, which turned out to be a good thing — some issues were more serious than I thought and vice versa.
        </p>
        <p class="paragraph-mobile-small desktop:paragraph-regular">
          This prioritized list helped me set the direction of my redesign. Feel free to check out the <a href="https://miro.com/app/board/o9J_loXL4Jk=/?moveToWidget=3458764514007662274&cot=14" target="_blank" class="text-link">“Issue Prioritisation” frame on my Miro project page</a> for more details
        </p>
      </div>
    </div>
    <div
      class="
        desktop:col-start-1 desktop:col-end-13
        col-span-full
        row-start-2
        text-left
      "
    >
      <div class="desktop:col-start-1 desktop:col-end-7">
        <h4 class="h4-medium text-gray-900">Competitive Analysis</h4>
        <div class="sm:px-3 sm:pl-8 sm:pr-4 mt-2">
          <p class="paragraph-mobile-small desktop:paragraph-regular">
            While working on affinity mapping, I’ve examined the websites of 4 competitors: Libri, Bookline, Líra and Agave Könyvek. I’ve analysed their home page, sign-up/sign-in process, checkout process, search and filtering options, product page, footer, and - if they had the option - how they handled requests for out-of-stock books. During this process, I once again took an abundance of screenshots, then used post-its to highlight positive and negative elements on each page, just like I did when analysing GABO Online at the beginning.
          </p>
          <p class="paragraph-mobile-small desktop:paragraph-regular">
            Neither of them is perfect, but each of them has some interesting solutions.
          </p>
          <p class="paragraph-mobile-small desktop:paragraph-regular">
            Next, I listed out the commonalities of these sites, to, once again, help me in my redesign. These 4 competitors are quite popular, especially Libri, Bookline and Líra — they mostly sell books published by them and by hundreds of other publishers, while Agave is a smaller but still popular publishing house, and their shop only sells their products, so I thought it provided a nice addition, as being somewhat similar to what GABO Online is.
          </p>
          <p class="paragraph-mobile-small desktop:paragraph-regular">
            In short, I’ve assembled their commonalities to have a more detailed list of what might be expected by or familiar to users on these kinds of e-commerce sites.
          </p>
          <p class="paragraph-mobile-small desktop:paragraph-regular">
            Feel free to check out the <a href="https://miro.com/app/board/o9J_loXL4Jk=/?moveToWidget=3458764514005635254&cot=14" target="_blank" class="text-link">“Competitive Analysis” frame on my Miro project page</a> for more details
          </p>
        </div>
      </div>
      <div class="desktop:col-start-1 desktop:col-end-7">
        <h4 class="h4-medium text-gray-900">Other Possible Steps</h4>
        <div class="sm:px-3 sm:pl-8 sm:pr-4 mt-2">
          <p class="paragraph-mobile-small desktop:paragraph-regular">
            If I had access to GABO’s real users, I would have run an online survey about their shopping habits, to find out their needs and pain points, especially since the group’s target audience is so wide — this step shouldn’t be missed if someone starts to redesign and rebrand GABO Online.
          </p>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  name: "GaboResearch",
};
</script>